...
public class BallMove : MonoBehaviour
{
    ...
    bool beforeClicked = false;
    private void FixedUpdate()
    {
        if (beforeClicked != cursor.GetComponent<CursorMovement>().clicked)
        {
            if (beforeClicked)
            {
                OnClickReleased();
            }
            else if (collider_.bounds.Contains(cursor.transform.position))
            {
                OnClick();
            }
            beforeClicked = cursor.GetComponent<CursorMovement>().clicked;
        }
    }
